...
//
// (Until 2026-08-13 this file also asserted the WORKER dial's reader in every
// broken-file case. The worker dial was deleted with the PTY worker runtime —
// workers are SDK-only, there is no worker reader left to pin — so the
// commander assertions below are the whole surviving contract.)
...
describe('the runtime dials at the FILE level — a broken settings.json is not consent', () => {
  it('A: no settings.json at all ⇒ commander sdk (the documented fresh-install default)', async () => {
