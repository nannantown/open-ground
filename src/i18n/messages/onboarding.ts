...
export const onboarding = {
  en: {
    ...
    'onboarding.setup.signinHint':
      'Nothing to do here — the first time you run Claude in OPEN GROUND it opens a browser to sign in (use a paid Claude plan: Pro, Max, Team, or Enterprise). This is a one-time step and your login is saved afterward.',
  ...
  ja: {
    ...
    'onboarding.setup.signinHint':
      'ここでの操作は不要です。OPEN GROUND で初めて Claude を使うとき、claude 自身がブラウザを開いてサインインを促します（有料プラン：Pro・Max・Team・Enterprise）。一度きりで、以降はログインが保存されます。',
