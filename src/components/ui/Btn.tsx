...
const BASE = 'inline-flex items-center justify-center gap-1.5 transition-colors disabled:cursor-not-allowed disabled:opacity-40'
...
export const Btn = forwardRef<HTMLButtonElement, BtnProps>(
  ({ variant = 'ghost', size = 'sm', flat, danger, className, children, ...rest }, ref) => {
    ...
    switch (variant) {
      case 'primary':
        variantCls = c(
          'bg-accent text-bg-card label-cap rounded-[2px] hover:bg-accent-hover',
          !flat && 'border border-accent-deeper shadow-card',
        )
        break
      case 'ghost':
        variantCls =
          'border border-line bg-transparent text-ink-muted label-cap rounded-[2px] hover:border-accent hover:text-accent'
        break
      case 'icon':
        variantCls = c(
          'rounded-sm',
