...
export const MANUAL_SECTIONS: Section[] = [
  // ───────────────────────────── 1 · What is it ─────────────────────────────
  {
    ...
    blocks: [
      ...
      { kind: 'diagram', id: 'layers' },
      {
        kind: 'p',
        text: {
          en: 'Ground shows your projects together. Open a project for Board and Terminal. Agent Team can be enabled in Settings on macOS; it lives in a bar along the bottom of every project. Canvas, Research and custom tabs are currently owner-only; previously saved data is retained.',
          ja: 'グラウンドでプロジェクトを一覧し、開くと Board・Terminal が使えます。macOS では設定からエージェントチームをオンにでき、各プロジェクトの一番下のバーに出ます。Canvas・Research・追加タブは現在オーナー向けですが、以前の保存データは残ります。',
        },
      },
      {
        kind: 'note',
  ...
  // ───────────────────────────── 2 · Quickstart ─────────────────────────────
  {
    ...
    blocks: [
      ...
      { kind: 'subhead', text: { en: '2 · Meet the three tabs', ja: '2 · 3つのタブを知る' } },
      {
        kind: 'p',
        text: {
          en: 'A project opens into Board (plan and run tasks) and Terminal (claude or a shell), with Agent Team as a folded bar along the bottom once enabled.',
          ja: 'プロジェクトでは Board（計画と実行）と Terminal（claude やシェル）を使えます。エージェントチームはオンにすると、画面の一番下に畳まれたバーとして出ます。',
        },
      },
  ...
  // ─────────────────────────────── 3 · Ground ───────────────────────────────
  {
    ...
    blocks: [
      { kind: 'subhead', text: { en: 'Cards & the beacon', ja: 'カードとビーコン' } },
      {
        kind: 'p',
        text: {
          en: 'Each card shows the project name, description, open-task count and a map coordinate, and at its top right at most one mark, with no words (hover it for a short name): a pulsing moss dot (the work is not finished yet — someone is working, a job is waiting for its check or being handed over, or, with Agent Team autopilot on, a queued job is about to start; it stays until everything is done, and jobs you parked do not count), a raised hand (a question is waiting for your answer — the president answered you with a question, or something only you can decide is open) or an eye (everything is finished and work was delivered since you last looked — just take a look). Opening the project counts as having looked and clears the eye; the hand does not go away by opening — answer the open decision, or open the Agent Team bar for the president’s question. The hand wins over everything, the moss dot over the eye. No mark means nothing needs you — look whenever you like.',
          ja: '各カードにはプロジェクト名・説明・未着手タスク数・地図座標が出て、右上に文字のない印が最大1つ付きます（マウスを乗せると短い名前が出ます）—— 苔色の点の点滅（作業がまだ終わっていない: だれかが作業中、確認待ち・受け渡しの最中、またはエージェントチームの自動運転中に着手待ちの作業がある。全部終わるまで出続けます。保留にした作業は数えません）、挙手の手（あなたの答えを待っている質問がある: あなたへの社長の返事が質問で終わっている、またはあなたにしか決められない質問が開いている）、目（全部終わって、前に見てから作業が納品された: 見るだけでよい）。プロジェクトを開けば「見た」ことになり目は消えます。手は開いただけでは消えません（あなたにしか決められない質問は答えると、社長の質問はエージェントチームのバーを開くと消えます）。手がいちばん優先、次が苔色の点、その次が目です。印が無ければ、見に行かなくて大丈夫です。',
        },
      },
      {
        kind: 'p',
    ...
    blocks: [
      {
        kind: 'p',
        text: {
          en: 'The Terminal tab tiles up to 5 panes side by side, each a real terminal opened in the project folder. Type `claude` yourself, or run a Board task to get a pre-configured claude session.',
          ja: 'Terminal タブは最大5ペインを横に並べます。それぞれがプロジェクトのフォルダで開かれた本物のターミナルです。自分で `claude` と打っても、Board のタスクを実行して設定済みの claude セッションを得てもかまいません。',
        },
      },
  ...
  // ───────────────────────────── 11 · Swarm ─────────────────────────────
  {
    ...
    title: { en: 'Agent Team — parallel Claude workers', ja: 'エージェントチーム — 並列 Claude ワーカー' },
    intro: {
    ...
    blocks: [
      ...
      { kind: 'subhead', text: { en: 'A bar under every tab, every seat inside', ja: 'どのタブでも一番下にバー・中に全員の席' } },
      {
        kind: 'p',
        text: {
          en: 'Agent Team is not a tab: it is a bar along the bottom of the project, the same bar whichever tab (Board, Canvas, Terminal…) you have open. It starts folded to one line: the name, how many questions are waiting on you and how many finished jobs await integration (each only when there are some) and the on/off switch at the right. Click anywhere on that line to open it upward (the switch itself only switches); grab that same line and drag it up or down to make the bar taller or shorter — a press without moving still just opens or folds it (the height is remembered per project). Whether it is open or folded is remembered per project too: leave a project with the bar open and it is open again when you come back, even after restarting the app; a project you have never opened it in starts folded. Inside, the president (the one you talk to), the manager, and one seat per worker sit side by side, each with its own character and a slightly different background colour per role. Each seat’s top line says who it is, what state it is in (working, waiting, needs an answer, stopped) and, for a worker, which job it is on. Below that, the manager’s seat and each worker’s seat show their recent conversation — what they were told, what they said and which tools they ran — updating by itself while they work and always following the newest line (scroll up to read back; “Latest” jumps down again). A worker’s seat also shows any question it is waiting on you for; “Watch the work” opens its fuller log (one worker at a time). Drag the line between two seats to change the width of the seat on its left; click that line, or a seat’s top line, to open that seat up wide (the others narrow), and click again to put it back (widths are remembered per project). When the seats no longer fit the width, the row scrolls sideways. The manager’s seat also says whether the manager is running, stopped or not there, with a quiet start/stop. At the bottom of the manager’s seat and of each working worker’s seat is a small “Send a word” link for the rare time you want to tell one of them something directly: it opens a box, and only the Send button or ⌘/Ctrl+Enter sends (Enter alone starts a new line). Normally you don’t need it — talk to the president. A word sent to a manager that isn’t there wakes it up. Monitoring (alerts about questions, stalled work and usage) and the working style (full power / economy / optimize) are not on the screen: ask the president in plain words — for example “turn monitoring on” or “switch to economy” — and it changes them for you and confirms. Which models may be used at all is in Settings.',
          ja: 'エージェントチームはタブではなく、プロジェクト画面の一番下のバーです。Board・Canvas・Terminal など、どのタブを開いていても同じバーが出ます。ふだんは1行に畳まれていて、名前・あなたへの質問の数と統合待ちの数(どちらもあるときだけ)と、右端にオン/オフのスイッチが見えます。この行のどこを押しても上に広がり(スイッチを押したときはオン/オフだけが切り替わります)、同じ行をつかんで上下にドラッグすると高さを変えられます —— 動かさずに押しただけなら、これまでどおり開くか畳むだけです（高さはプロジェクトごとに覚えます）。開いているか畳んでいるかもプロジェクトごとに覚えます —— 開いたままグラウンドに戻っても、アプリを再起動しても、次に入ったときは開いたままです。一度も開いていないプロジェクトは畳んだ状態で始まります。中では、社長（あなたが話す相手）・マネージャー・ワーカー1人につき1席が横に並び、役ごとにキャラクターと少し違う背景色が付いています。各席の一番上の行に、誰か・いまの状態（作業中・待機中・回答待ち・停止中）・ワーカーならどの仕事をしているかが出ます。その下には、マネージャーの席と各ワーカーの席に、直近のやりとり(受け取った指示・話していること・使った道具)がそのまま出ます。動いている間は自動で更新され、いつも最新の行が見えるように追いかけます(上にスクロールすると読み返せて、「最新へ」で戻れます)。ワーカーの席には、あなたへの質問があればそれも出ます。「作業の様子を見る」を押すと、その作業のくわしい記録を開けます（一度に1人ずつ）。席と席の境目をドラッグすると、その左の席の幅を変えられます。境目か席の一番上の行を押すと、その席が大きく広がり（ほかの席は狭まります）、もう一度押すと元に戻ります（幅はプロジェクトごとに覚えます）。席が横幅に収まらなくなると、列が横にスクロールします。マネージャーの席には「動いている・止まっている・いない」と控えめな起動・停止も出ます。マネージャーの席と、動いているワーカーの席の一番下には小さな「一言送る」があり、どうしても直接伝えたいときだけ使えます。押すと入力欄が開き、「送る」ボタンか ⌘/Ctrl+Enter でだけ送られます(Enter だけなら改行です)。ふだんは使う必要はありません —— 社長に話してください。いないマネージャーに送ると、マネージャーを起こして届けます。「状況の監視」（質問・作業の停滞・利用上限の通知）と働き方（最大出力・節約・最適化）は画面にはありません。社長に言葉で頼んでください —— 例えば「状況の監視をオンにして」「節約にして」と言えば、社長が切り替えて結果を確かめて伝えます。どのモデルを使ってよいかは設定画面にあります。',
        },
      },
      { kind: 'subhead', text: { en: 'What starts on its own — and what never does', ja: 'ひとりでに始まるもの、絶対に始まらないもの' } },
      {
        ...
        items: [
          {
            en: 'Worker dispatch (drain): starts only when you turn the switch on the Agent Team bar on. It stops immediately when you turn it off, and stays stopped across a restart — it never auto-resumes. (That "never auto-resumes" is about a project you explicitly stopped — it still applies. A project you had left switched ON is different: it now resumes automatically after a restart, with no action from you, unless you had stopped it or the app itself has been restarting repeatedly, in which case it stays off and you get a notification.)',
            ja: 'worker 起動（drain）：エージェントチームのバーのスイッチをオンにしたときだけ始まります。オフにすれば即座に止まり、再起動後も停止したままです —— 自動では再開しません。（この「自動では再開しません」は、あなたが明示的に停止していたプロジェクトについての話で、これは今も変わりません。一方、オンにしたままだったプロジェクトは話が別です —— アプリの再起動後、あなたが何もしなくても自動的に再開するようになりました。停止していた場合や、アプリ自体が短時間に繰り返し再起動している場合は例外で、その場合はオフのままになり通知が届きます。）',
          },
          {
            en: 'Integration (landing finished work on your trunk): the engine itself never pushes — it has no code path that moves your trunk, and regression tests pin that. The one thing it does on its own while running: when a worker finishes and its card reaches review, it wakes the manager — a `claude` session you can watch in the Agent Team bar — if none is alive. Waking moves nothing by itself. Your trunk changes only when that manager session reviews the branch and lands it with a plain push (never `--force`); a conflicting push is aborted and nothing lands, and cards titled `[hold]` or touching high-risk paths are always held for your explicit approval.',
            ja: '統合（完了した作業を本流へ取り込む）：エンジン自身は push しません —— エンジンには本流を動かすコード経路そのものが無く、回帰テストがそれを固定しています。稼働中にひとりでに行うのは1つだけ：worker が作業を終えカードが review 列に届いたとき、マネージャー（エージェントチームのバーで見られる `claude` セッション）の卓が不在なら起こします。起こすこと自体では何も動きません。本流が変わるのは、そのマネージャーセッションがブランチをレビューし、plain push（`--force` は一切なし）で取り込んだときだけです。衝突した push は中断され、何も取り込まれません。タイトルが `[hold]` のカードや高リスクなパスに触れた変更は、常にあなたの明示承認待ちとして保留されます。',
          },
          {
      ...
      },
      {
        ...
        tone: 'warn',
        text: {
          en: 'One more thing to know, and it depends on which computer you use. Agent Team’s always-on brake is a rule-checker: it refuses dangerous commands before they run. On macOS there is a second brake behind it — macOS itself — and it is worth being precise about what that one does and does not do. It stops a session from writing anywhere outside its own work folder, and it makes certain secret files unreadable: SSH keys, cloud sign-in credentials, the browser password databases. It does not hide your ordinary files: your documents and the rest of your home folder stay readable. And items saved in the macOS keychain stay within reach on purpose — blocking those stops Claude from starting at all — so a session that goes wrong can still ask the keychain for saved passwords. On Windows that second brake does not exist at all. There is nothing to switch on, so the rule-checker is on its own — and it has not yet been tried on a real Windows machine, so we cannot promise how it behaves there. So on Windows, assume a worker that goes wrong can also read the secret files macOS would have blocked, and can change files anywhere you could. (One protection is the same on every computer: the overseer always runs with a fixed set of tools switched off, and that cannot be overridden.) Practical advice: prefer macOS if you want to leave Agent Team running while you are away — but “while you are away” still means “worth checking on”, because even there a worker can read your ordinary files and reach saved keychain items. (Detail: the OS-level layer is macOS Seatbelt; there is no Windows equivalent in the app — docs/SANDBOX_EXPERIMENT.md, GAP-7, decided 2026-07-27.)',
          ja: 'もう1つ、お使いのパソコンによって変わる話があります。エージェントチームに常時かかっているブレーキは「規則の見張り」です —— 危ない操作を、実行される前に拒否します。macOS にはその後ろにもう1枚、macOS 自身によるブレーキがあります。ただし、それが何をして何をしないかは正確に知っておいてください。これはまず、そのセッションが自分の作業フォルダの外へ書き込むのを止めます。そして特定の秘密ファイルを読めなくします —— SSH 鍵、クラウドのログイン情報、ブラウザのパスワード保管ファイルです。一方で、それ以外のふつうのファイルは隠しません：書類フォルダやホームの残りは読める状態のままです。さらに、macOS キーチェーンに保存された項目は意図的に届く範囲に残されています（塞ぐと Claude がそもそも起動できなくなるためです）—— つまり、おかしくなったセッションはキーチェーンに保存済みのパスワードを問い合わせることができます。Windows にはこの2枚目がそもそも存在しません。オンにできるものが無いので「規則の見張り」1つだけになり、しかもその見張りは Windows の実機でまだ試されていないため、そこでどう振る舞うかをお約束できません。ですから Windows では、おかしくなった worker は「macOS なら塞がれていた秘密ファイルまで読める」「あなたが書き換えられる場所はどこでも書き換えられる」と考えてください。（どのパソコンでも同じ守りが1つあります：監督役は決められた道具をオフにした状態で必ず動き、これは上書きできません。）実務的な助言：席を離れている間もエージェントチームを走らせておきたいなら macOS を選んでください —— ただし「席を離れている間」は「見に行かなくてよい」ではありません。macOS でも worker はふつうのファイルを読め、キーチェーンの保存項目には手が届きます。（技術的な詳細：OS 側の層は macOS の Seatbelt で、Windows 相当はアプリに実装していません —— docs/SANDBOX_EXPERIMENT.md・GAP-7・2026-07-27 決定。）',
        },
      },
      {
        kind: 'note',
