  ...
  // ───────────────────────────── 1 · What is it ─────────────────────────────
  {
    ...
    blocks: [
      ...
      { kind: 'diagram', id: 'layers' },
      {
        kind: 'p',
        text: {
          en: 'Ground shows your projects together. Open a project for Board and Terminal. Swarm can be enabled in Settings on macOS; it lives in a bar along the bottom of every project. Canvas, Research and custom tabs are currently owner-only; previously saved data is retained.',
          ja: 'グラウンドでプロジェクトを一覧し、開くと Board・Terminal が使えます。macOS では設定から Swarm をオンにでき、各プロジェクトの一番下のバーに出ます。Canvas・Research・追加タブは現在オーナー向けですが、以前の保存データは残ります。',
        },
      },
      {
        kind: 'note',
  ...
  // ───────────────────────────── 2 · Quickstart ─────────────────────────────
  {
    ...
    blocks: [
      ...
      { kind: 'subhead', text: { en: '2 · Meet the three tabs', ja: '2 · 3つのタブを知る' } },
      {
        kind: 'p',
        text: {
          en: 'A project opens into Board (plan and run tasks) and Terminal (claude or a shell), with Swarm as a folded bar along the bottom once enabled.',
          ja: 'プロジェクトでは Board（計画と実行）と Terminal（claude やシェル）を使えます。Swarm はオンにすると、画面の一番下に畳まれたバーとして出ます。',
        },
      },
  ...
  // ───────────────────────────── 11 · Swarm ─────────────────────────────
  {
    ...
    blocks: [
      { kind: 'subhead', text: { en: 'Turning it on', ja: '有効化する' } },
      {
        kind: 'p',
        text: {
          en: 'Swarm is hidden by default. Enable it from Settings → Experiments → Swarm orchestration. This only reveals the Swarm bar and its controls for you — nothing runs automatically. Every autonomous action inside stays off until you explicitly arm it. Once armed, worker dispatch now survives an app restart — see the next section for exactly what that means and does not mean.',
          ja: 'Swarm は既定では非表示です。設定 → 実験的機能 → Swarm オーケストレーション で有効化してください。これは Swarm のバーと操作を可視化するだけで、それ自体では何も自動実行されません。中の自律的な操作はすべて、あなたが個別に明示オンにするまで動きません。一度オンにすると、worker 起動はアプリの再起動をまたいで生き残るようになりました —— 具体的に何がどうなるかは次の節で説明します。',
        },
      },
      { kind: 'subhead', text: { en: 'A bar under every tab, every seat inside', ja: 'どのタブでも一番下にバー・中に全員の席' } },
      {
        kind: 'p',
        text: {
          en: 'Swarm is not a tab: it is a bar along the bottom of the project, the same bar whichever tab (Board, Canvas, Terminal…) you have open. It starts folded to one line that says whether it is running, how many workers there are and how many questions are waiting on you, with the on/off switch at the right. Click “Swarm” to open it upward; drag its top edge to make it taller or shorter (the height is remembered per project). Inside, the president (the one you talk to), the manager, and one seat per worker sit side by side, each with its own character and a slightly different background colour per role. Each seat’s top line says who it is, what state it is in (working, waiting, needs an answer, stopped) and, for a worker, which job it is on. A worker’s seat shows just that summary and any question it is waiting on you for; press “Watch the work” to open its live log (one worker at a time). When the seats no longer fit the width, the row scrolls sideways. The manager’s seat is deliberately small: it only says whether the manager is running, stopped or not there, with a quiet start/stop — you never need to talk to it. The “Monitoring” switch (questions, stalled work and usage alerts) sits on the bar’s top line next to the on/off switch; it turns off whenever the swarm stops or the app restarts, so switch it back on when you want it.',
          ja: 'Swarm はタブではなく、プロジェクト画面の一番下のバーです。Board・Canvas・Terminal など、どのタブを開いていても同じバーが出ます。ふだんは1行に畳まれていて、動いているか・ワーカーの人数・あなたへの判断待ちの数と、右端にオン/オフのスイッチが見えます。「Swarm」を押すと上に広がり、上端をドラッグすると高さを変えられます（高さはプロジェクトごとに覚えます）。中では、社長（あなたが話す相手）・マネージャー・ワーカー1人につき1席が横に並び、役ごとにキャラクターと少し違う背景色が付いています。各席の一番上の行に、誰か・いまの状態（作業中・待機中・回答待ち・停止中）・ワーカーならどの仕事をしているかが出ます。ワーカーの席はその要約と、あなたへの質問があればそれだけを出します。「作業の様子を見る」を押すと、その作業の記録を開けます（一度に1人ずつ）。席が横幅に収まらなくなると、列が横にスクロールします。マネージャーの席はわざと小さくしてあり、「動いている・止まっている・いない」と控えめな起動・停止だけを出します —— マネージャーに話しかける必要はありません。「状況の監視」スイッチ（質問・作業の停滞・利用上限の通知）はバーの上の行、オン/オフのスイッチの隣にあります。Swarm を止めたときやアプリを再起動したときはオフに戻るので、使いたいときにもう一度オンにしてください。',
        },
      },
      { kind: 'subhead', text: { en: 'What starts on its own — and what never does', ja: 'ひとりでに始まるもの、絶対に始まらないもの' } },
      {
        ...
        items: [
          {
            en: 'Worker dispatch (drain): starts only when you turn the switch on the Swarm bar on. It stops immediately when you turn it off, and stays stopped across a restart — it never auto-resumes. (That "never auto-resumes" is about a project you explicitly stopped — it still applies. A project you had left switched ON is different: it now resumes automatically after a restart, with no action from you, unless you had stopped it or the app itself has been restarting repeatedly, in which case it stays off and you get a notification.)',
            ja: 'worker 起動（drain）：Swarm のバーのスイッチをオンにしたときだけ始まります。オフにすれば即座に止まり、再起動後も停止したままです —— 自動では再開しません。（この「自動では再開しません」は、あなたが明示的に停止していたプロジェクトについての話で、これは今も変わりません。一方、オンにしたままだったプロジェクトは話が別です —— アプリの再起動後、あなたが何もしなくても自動的に再開するようになりました。停止していた場合や、アプリ自体が短時間に繰り返し再起動している場合は例外で、その場合はオフのままになり通知が届きます。）',
          },
          {
            en: 'Integration (landing finished work on your trunk): the engine itself never pushes — it has no code path that moves your trunk, and regression tests pin that. The one thing it does on its own while running: when a worker finishes and its card reaches review, it wakes the manager — a `claude` session you can watch in the Swarm bar — if none is alive. Waking moves nothing by itself. Your trunk changes only when that manager session reviews the branch and lands it with a plain push (never `--force`); a conflicting push is aborted and nothing lands, and cards titled `[hold]` or touching high-risk paths are always held for your explicit approval.',
            ja: '統合（完了した作業を本流へ取り込む）：エンジン自身は push しません —— エンジンには本流を動かすコード経路そのものが無く、回帰テストがそれを固定しています。稼働中にひとりでに行うのは1つだけ：worker が作業を終えカードが review 列に届いたとき、マネージャー（Swarm のバーで見られる `claude` セッション）の卓が不在なら起こします。起こすこと自体では何も動きません。本流が変わるのは、そのマネージャーセッションがブランチをレビューし、plain push（`--force` は一切なし）で取り込んだときだけです。衝突した push は中断され、何も取り込まれません。タイトルが `[hold]` のカードや高リスクなパスに触れた変更は、常にあなたの明示承認待ちとして保留されます。',
          },
          {
