  ...
  // ───────────────────────────── 11 · Swarm ─────────────────────────────
  {
    ...
    blocks: [
      ...
      { kind: 'subhead', text: { en: 'One screen, every seat', ja: '1画面に全員の席' } },
      {
        kind: 'p',
        text: {
          en: 'The Swarm tab is one screen with no sub-tabs: the president (the one you talk to), the manager, and one seat per worker sit side by side, each with its own character and a slightly different background colour per role. Each seat’s top line says who it is, what state it is in (working, waiting, needs an answer, stopped) and, for a worker, which job it is on. A worker’s seat shows just that summary and any question it is waiting on you for; press “Watch the work” to open its live log (one worker at a time). When the seats no longer fit the width, the row scrolls sideways. The manager’s seat is deliberately small: it only says whether the manager is running, stopped or not there, with a quiet start/stop — you never need to talk to it. The “Monitoring” switch (questions, stalled work and usage alerts) sits on the top bar next to Start/Stop; it turns off whenever the swarm stops or the app restarts, so switch it back on when you want it.',
          ja: 'Swarm タブはサブタブの無い1画面です。社長（あなたが話す相手）・マネージャー・ワーカー1人につき1席が横に並び、役ごとにキャラクターと少し違う背景色が付いています。各席の一番上の行に、誰か・いまの状態（作業中・待機中・回答待ち・停止中）・ワーカーならどの仕事をしているかが出ます。ワーカーの席はその要約と、あなたへの質問があればそれだけを出します。「作業の様子を見る」を押すと、その作業の記録を開けます（一度に1人ずつ）。席が横幅に収まらなくなると、列が横にスクロールします。マネージャーの席はわざと小さくしてあり、「動いている・止まっている・いない」と控えめな起動・停止だけを出します —— マネージャーに話しかける必要はありません。「状況の監視」スイッチ（質問・作業の停滞・利用上限の通知）は上部バーの開始/停止の隣にあります。Swarm を止めたときやアプリを再起動したときはオフに戻るので、使いたいときにもう一度オンにしてください。',
        },
      },
