...
- **settings.json が破損して読めない場合、フラグは既定値 OFF(= 外部通信が復活)へ
  ...
  **追記(2026-08-02)**: 破損している間は**設定の保存そのものが拒否される**ように
  なった(`SettingsUnreadableError`)。したがって「壊れたまま lockdown を ON にする」は
  できず、**先にファイルを直す/消す必要がある**。これは後退ではなく保護 —— 従来は
  保存が通る代わりに、読めない現在値が既定値に潰れて書き戻され、**登録プロジェクト
  一覧(`projects`)が丸ごと消えていた**(実測 2026-08-02)。フラグ自体の読みは
  従来どおり「壊れたら OFF」で三者一致(この段落の上記は不変)。
