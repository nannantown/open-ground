...
worker が生まれる経路は (a) エンジンの dispatch パス(swarmOrchestrator.ts:4562 `deps.spawnWorker` → :2313 `defaultSpawnWorker`)、(b) 手動/司令塔の `POST /api/swarm/worker`(server/routes/swarm.ts:241)、(c) UI の Restart(SwarmModule.tsx:714 — 同じ route に `worktree` 付きで POST)。**全経路が `spawnSwarmWorker`(swarmWorker.ts:455)に合流する。**
...
### 2.5 guard(SWARM_MANAGER と OPENGROUND_GUARD は別物)

環境変数で 2 系統の deterministic veto が仕掛けられている。**worker と司令塔で使われる系統が違う**:

- **worker**: `guard: { writeRoots: [worktree] }`(swarmWorker.ts:423)→ launchClaude が `OPENGROUND_GUARD=1` + `OPENGROUND_GUARD_WRITE_ROOTS=<worktree>` を注入(claudeTerminal.ts:535-543)。グローバル配線済みの PreToolUse hook(openground-guard.js)が exit 2 で deny — bypass(--dangerously-skip-permissions)でも上書きできない唯一の veto。Write/Edit/Bash の書き込みを worktree 内に閉じ込め、`git push` は全形態 deny。共有 `.git` は意図的に writeRoots に**入れない**(:423 コメント — git はバイナリ経由で動くので Bash ルールが統治する。root にすると .git への生リダイレクトまで正当化してしまう)
- **司令塔/補給官**: `SWARM_MANAGER=1`(swarmSupply / swarmManager が設定 — server/routes/swarm.ts:377, :416 コメント)。本体 checkout で動く役割向けの破壊的 git ブロック。**worker は env を渡さないので SWARM_MANAGER 系 guard は不発のまま**(swarmWorker.ts:354-355, claudeTerminal.ts:103-109)
