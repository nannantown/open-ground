...
- **hook source の解決は cwd 非依存 + worktree root 拒否(2026-07-12 根治)**: かつて `installHooks()` は hook/guard スクリプトを `process.cwd()/scripts/` から解決していたため、**cwd が swarm worktree を指した状態で install が走ると、その worktree の絶対パスがグローバル `~/.claude/settings.json` に焼き込まれ、janitor が worktree を消した時点で全 claude セッション(OPEN GROUND と無関係なプロジェクト含む)の Stop hook が MODULE_NOT_FOUND で壊れる**実事故が起きた(0712 観測 — 例: worktree 内で起こした dev サーバの boot install)。現在は `resolveHookSourceRoot()`(hooksInstall.ts)が**モジュール自身の位置**を起点に scripts/ 2 本を持つディレクトリまで上方探索して解決(dev/vitest = checkout root、esbuild bundle = app root — cwd は一切見ない)。さらに belt-and-braces として、解決 root が **OPEN GROUND データホーム(`~/.openground`)配下なら fail-closed で拒否**(swarm worktree の住処 = janitor が消す揮発領域): `installHooks` は何も書かずに `errors` 報告、`verifyGuardWiring` も NOT-ok(= worktree 内で動くエンジンからの worker spawn は拒否)。`installOgManageSkill`(og-manage スキルの boot install)も同じ resolver 経由。回帰ネット: **hooksInstall.test.ts**(R1: cwd=worktree でも worktree パスが settings.json に載らない / R2·R2b: worktree 常駐エンジンは何も書かない・既存 settings byte 不変 / R3: verify が worktree source を期待版と誤認しない / R4: 解決の cwd 非依存)
