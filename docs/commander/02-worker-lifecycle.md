...
>
> **追記(2026-09-22・補給官の卓に知らせが届くようになった)**: 同じ「空いた時だけ書く」判定(`noticeDeliverable` — 生成中/入力途中/メニュー表示中は書かない)を使って、エンジンが**オーナーの判断か認知が要る4種の知らせ**(質問が開いた/高リスクで統合が止まった/カードが本体に入った/fatal)を補給官の卓に1行打つ。1枠・新着上書き・届いたら即クリアで、再送は上の 60s ループに相乗りする(専用ポーリングは無い)。これでオーナーは司令官の窓を見に行かずに済む。仕様と番人は 06 章 §1.4、実装は `src/lib/server/supplyNotice.ts`。
> エンジンログに「卓を作り直した(文脈 N から 0 へ)」「卓を圧縮した(文脈 N から M へ)」が出る。正典は 05 章 §10.6。
...
| positional prompt | swarmWorker.ts `buildOrderInjection`(呼び手は swarmWorkerSdk.ts の起動計画) | `/order ゴール: …` を**起動時引数**として渡す(後述)。**2026-09-22〜 ゴールの直後に実効の難易度型の `【難易度: <型>】` 方針文が付く**(サブエージェント数と敵対レビューの有無を型で縛る — 04 章 §5.9 追記 2026-09-22) |
...
**(g) subagent オフロード(2026-07-28 追加、日次燃費日報の劣化起票カードから。上の6項目に足して7項目)**: 着手前に**日報と同じ式・同じ窓で read-only 再計測**したところ(窓は日報と同じく `MAX_WINDOW_MS = 26h` でクランプした範囲 — `dailyFuelReport.ts:74`。文脈max は窓内の**最大値**なので窓を伸ばすほど基準を跨ぎやすい = 窓を揃えないと悪化して見える)、(a) の既定反転を入れた後も **束ね率 1.185(基準1.3割れ)/ 文脈max 39.2万(基準30万超)** で**2項目とも基準割れのまま**だった(手数中央値 87.5 は基準150以内 = 健全なので**触っていない**)。決定打は別の数字で、**窓内8カードすべてが `sidechainOutputTokens === 0`** — つまり誰ひとり調べものを外に出していなかった。そこで (g)「長い調べものは自分で読まず subagent に投げる」を追加した。**どの指標にどう効くか(⚠ 一方向ではない — 束ね率は混在)**: 計量器は sidechain 応答を **turns / toolTurns / toolUses / maxContext のすべてから除外**する(`swarmTokenAudit.ts` — sidechain は加算前に `continue`)。ただし**そこから先は指標ごとに向きが違う**(この段落の初版は「必ず上がる」と一方向に書いていた — **誤り**。2026-07-28 敵対レビュー MUST-FIX 1 で差し戻し): ① **文脈max は全トリガで下がる** — subagent が読んだ中身は親の文脈に一切積まれない。これが (g) の**無条件の効き目**で、燃費カードの「文脈」項目の解はここ。② **束ね率は上下する**。**トリガ②(当たりが絞れない探索 = 本当に逐次)は上げ側**で、単発になりがちな調査手 k 個を Task 1手へ畳むと超過分 `toolUses − toolTurns` は不変のまま分母だけ k−1 縮み、比 `1 + 超過/分母` は上がる(窓の最悪カードで 239/210 = 1.14 → 190/161 = 1.18。⚠ **後半は実測ではなく試算** — そのカードの210手のうち49手=23%が畳める単発だった、と仮定した数字)。**トリガ①(3ファイル以上の横断)は下げ側**で、①の反実仮想は「単発 b 手」ではなく **(a) が生むはずだった b 個入りの1応答**(`toolTurns +1` / `toolUses +b`)である。それを Task に回すと同じ1応答が道具1つ(`toolTurns +1` / `toolUses +1`)になり、**分母は動かず分子だけ b−1 減る** → 比は `n(b−1)/T` だけ**下がる**(1.185 から、b=3 が道具入り応答の3%で発火すると **1.125**、b=4 なら **1.095**)。しかも①は**必須かつ (a) より具体的なので b≥3 では (g) が勝ち**、**(a) が生むべき最も束ね率の高い応答をちょうど食う**。(a) 配線の一文(「独立した調べものが複数あるなら Task も同じ応答に」)は救いにならない — あれは**独立した調査が複数同時**のときだけ効き、①は**1つの調査が3+ファイルに跨る**ときに発火するので Task は1本しか出ない。**だから条文に優先関係を1文入れてある**: 「①が (a) とぶつかったら (a) が勝つ — 読む場所が既に file:line で特定できているなら投げずに (a) どおり束ねて読め。①が発火するのは『どこを読めばいいか分からないから読む』= 探索になる時だけ」。これで下げ経路は**探索的な読みだけに絞られる**(そこは親がどうせ単発を何手も費やす場面なので上げ側に回る)が、**消えたわけではない** — b≥3 の探索読みを束ねられた worker は依然 b−1 を失う。⚠ **正味の符号は定義からは決まらない。判定するのは翌日以降の日次燃費日報だけで、それまで (g) は「束ね率が落ちたときの容疑者」に残すこと**(ここは診断の正典なので、将来の司令官に「(g) は比を上げるだけ」と読ませない=容疑者リストから外させないのが要件)。**「使ってよい」ではなく明示指示にした理由**: worker を動かす harness の既定は「ユーザーが要求しない限り subagent を控える」で、worker にとっての「ユーザー」はこの注入文そのもの。許可の書き方では発火しない — **現に既存の【技術判断は一次資料で】節にある「重い調査は sub-agent(Task)に投げて要点だけ受け取れ」は8カード連続で sidechain 0**(一次資料の取り込みという狭い文脈に閉じており、かつ「重い」の判定を worker に委ねている)。**トリガを観測可能な3条件**(①3ファイル以上の横断読み ②grep の当たりが絞れない探索 ③ログ・テスト出力・大きい生成物の読み解き)にしたのは (a) の書き換えと同じ理由で、「重い調査なら」のような主観ゲートは**worker が既に気づけた時にしか発火しない**から。**完了ゲート3点と ready 前セルフコミットは今回も不変**(外に出すのは**調査だけ**で、判断・実装・検証は worker 自身の仕事のまま)。ピンは機構に張る(`swarmWorker.test.ts` の `pins the SUBAGENT-OFFLOAD mechanism of (g), not just its heading`) — ①明示指示であること ②観測可能トリガ3つ ③要点だけ受け取る(全文を戻させない) ④(a) との優先関係、の**4点を (g) 節の中で**見る。加えて **(a) との配線**(「独立した調べものが複数あるなら Task も同じ応答にまとめて出す」)も **(g) スライスに対して** assert する — 全体に対して見ていると、その一文が別節へ移動しても緑のまま通るからだ。

⚠ **スライスの閉じ位置がピンの生死を決める**(2026-07-28 敵対レビュー MUST-FIX 2 の実測): 初版は (g) の終端を**固定文字列(節末の完了ゲート文)**にアンカーしていた。そのため **(g) を見出しだけにして機構を丸ごと新設 `(h)` へ移す**書き換えが **51 passed / 0 failed で素通り**した — テストのコメントと本節が「防ぐ」と明言していた**まさにその失敗**が、書かれたとおりには偽だった。**初版の6変異はすべてスライスの内側**を突いており、境界そのものを試していない((a) のピンが殻のまま生きていたのと同じ死角)。修正は `tokenClause()` で、**次の lettered 条文で閉じる**((a) が `' (b) '` で閉じているのと同じ形を、次条文がまだ無い**最終条文**でも成り立つよう一般化したもの)。探すのは **letter より後ろの英字だけ**にしてある — 条文本文が他条文を「(a)」と参照するので `[a-z]` で探すとその参照でスライスが早期に閉じる。併せて「スライスが後続条文を飲み込んでいない」ことも直接 assert した。**教訓: 節を切り出すピンは、中身の変異だけでなく境界の変異でも試すこと**(中身の変異は全部赤でも、境界が緩ければ機構は丸ごと外へ持ち出せる)。

⚠ **ピンが本当に効くかは変異実測で確かめてある**(2026-07-28・**隔離 HOME/OPENGROUND_HOME 下**で実行 / baseline 51 passed): **①明示指示を「使ってもよい」に薄める / ②トリガを「重い調査だと思ったら」に差し戻す / ③要点だけ受け取るを削除(全文返却の解禁) / ④優先関係を削除 / (a)配線の削除 / 境界(投げるのは調査だけ)の削除** がいずれも **1 failed**、**見出しだけ残す殻化**と**機構を `(h)` へ移す境界変異**が **2 failed** — **8変異すべて赤**(緑のまま素通りする変異は無い)。さらに**同じ境界変異を差し戻し前のテスト(95829188)に対して回すと ALL GREEN** で、**穴と修正の因果**まで確認済み。効果は翌日以降の日次燃費日報が判定する(このカードの中では待たない)。
...
1. **エンジン roster**(:175-195): `getOrchestratorState` 経由。richest(taskId/taskTitle/startedAt/stage)。`heartbeatAt` は **`hb?.updatedAt ?? w.heartbeatAt`**(:188、2026-07-11 修正) — ディスクの心拍を優先し、無い場合だけエンジン in-memory の凍結値へフォールバック。`ready`/`blocked`/`blockers` も同じ `hb`(ディスクの心拍)から取る(:189-193)ので、1 レコード内の鮮度混在は解消済み。`phase`/`note` はエンジンの凍結値のまま(修正対象外)。**ハンドル(`terminalId` / `sdkSessionId`)は両方とも、その時点でプールに実在する時だけ出す**(2026-09-17 修正) — `terminalId` は以前から PTY プールを cwd で引き直していたが、`sdkSessionId` は roster の記録を無条件にコピーしていた(非対称)。roster は monitor が回収するまで残るので、セッションが終わった SDK worker が「届かないハンドル」を公開し続け、「ハンドルがある=生きている」と読む consumer が死んだ worker を生きていると描く経路だった。今は `liveCwdToSdkId.get(w.worktree)` で引く(番人: swarmWorkerRegistry.test.ts「GONE from the pool」・修正前で赤を実測)。⚠ ただし本番の `getOrchestratorState` は `stateOf` の `isAlive`(SDK 側 = `!reaped`)で roster を絞ってから渡すので、この非対称**単独**では sns-hub の Running 誤表示は再現しない — 実測の症状は下の「Ground カードの Running」段落
...
**Ground カードの「Running」は「ハンドルがある」ではなく「そのセッションが working」で決まる(2026-09-17 実測・修正)**: sns-hub で、唯一の worker が `phase:done`・`ready:true`・心拍 3.5 時間前、SDK セッションは**プールに生存**(`GET /api/terminal/active` で status `waiting` = ターン終了・統合待ち・未 reap)のまま、Ground カードが Running を出し続けた(オーナーが「なぜrunningになっている?」と2回確認)。`groundLamps.ts` の `liveWorkForProject` が「worker がハンドルを持つ=動いている」と読んでいたのが原因 — 仕事を終えて統合待ちの SDK worker は司令官が統合するまでプールに常駐するので、その間ずっと Running になっていた。今は worker のハンドル(runtime が名指す1本 `sdkSessionId || terminalId`)を `listAllActiveDesks()`(両プール)の pane に引き当て、その status が `working` の時だけ数える — オーナー自身の pane に 2026-08-17 から適用している物差しと同じ。`waiting`(終了・質問中・quota-parked)/`idle`/どのプールにも無い、は数えない。`groundLamp.ts` の4ケース(オーナー設計 2026-08-15/18)は不変。番人: groundLamps.test.ts「resident but PARKED」「NEITHER pool」(修正前で赤を実測)。裏取りコマンド: `curl -s "http://127.0.0.1:47776/api/ground/lamps" | jq -c '.lamps[] | select(.liveWork)'` と `curl -s http://127.0.0.1:47776/api/terminal/active | jq -c '.claude[] | {id, cwd, status}'` を突き合わせる(Running なのに該当 cwd の status が `working` でなければ再発)。
...
2. **外部差し戻しの観測**(2026-07-13 追加。**現存する唯一の経路**) — 司令官の `POST /api/project/tasks {rework:[…]}` や UI ドラッグ(review→doing)は **in-memory roster に届かない**。monitor は「roster が `stage:'done'` なのにカードが `doing` に戻っている」形を外部差し戻しとみなし、`stage='running'` + `reworkAt = now`(観測 tick の時刻 — Board の rework verb はカードに差し戻し時刻を記録しないため)で再武装して通常の監視フローに落とす(:5177-5190。engine log に `Board 側の差し戻し(review→doing)を観測` が 1 回出る)。**この観測が無かった時代は直後の stage:'done' 早期 continue(:5209-5233)が先に効いて永久スキップ** — worker が直して ready を打ち直してもカードは doing に沈み続けた(2026-07-13 実測: 55 分放置しても昇格せず、司令官の手動 setColumn でしか復旧しなかった)。
   ...
   **観測は worker 自身にも1通届く**(2026-08-03 追加)。再武装は監視側の帳簿で、worker は何も聞いていなかった — 司令官発の差し戻しは司令官が自分で卓に伝えるからよいが、**オーナーの UI ドラッグ発だと誰も伝えず**、worker(特に SDK — terminalId が空で paste-task が届かず、生きているがゆえに worktree 再起動は占有ガードに拒否される)は無効化済みの ready に向かって働き続けた(0803 実測)。観測ブロックが `deliverAnswerToWorker`(swarmEscalations の両ランタイム対応の配達口 — SDK は `pushSdkInput` で1ターン queue・PTY は3条件ガード付き注入)で「差し戻し・以前の ready は無効・notes を読み直して対応→再 ready」を1通送る。engine log に `差し戻しを worker の卓に伝えました`(成功・info)/`伝えられませんでした`(失敗・warn — 卓が busy/死亡。ceiling/nudge が後ろに立つので再送ループはしない)。観測1回につき1通(stage が flip するので連打しない)。回帰: swarmOrchestrator.test.ts「外部差し戻し…」+「差し戻し通知が届かなくても観測は成立する」(除去変異で赤を実測済み)。
