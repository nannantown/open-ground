...
### 1.5 司令官 → 補給官の**返事**(2026-09-22・オーナー決定「タスク窓口が社長」)

**なぜ在るか。** §1.4 で引いたのは**エンジン**→補給官。司令官→補給官は依然として無かった。
補給官は `POST /api/swarm/manager/say` で司令官に**話しかけられる**が、司令官の答えは司令官自身の
窓に出るだけで誰も読まない。つまりオーナーは「入れて」は言えて「**入れて大丈夫か聞いて**」が言えない
— 往復の帰りの経路が無い。この1本が、補給官を「オーナーが話す唯一の席」にできなかった理由だった。

| 性質 | 実体 |
|---|---|
| 入口 | `POST /api/swarm/supply/say` body `{path, text}`(owner gate + path validate。`server/routes/swarm.ts`) |
| 実装 | `supplyNotice.ts` の `queueSupplyReply` — 知らせの枠とは**別レーン** |
...
**司令官が居なければ起こす**(同日・オーナー決定「いいよ。おこして。」)。`manager/say` は以前
「卓が無ければ 404、起こすのはエンジンの反射かオーナーのボタン」だった。コストの判断は正しかったが
**誰が決めるか**を取り違えていた — 司令官は ready なカードがある時だけエンジンに起こされるので
日中はたいてい不在で、返事の経路が通った後は「司令官がいません」がほぼ全部の質問への答えになる。
`wake:false` で従来動作。`managerDesired` は**立てない**(質問は一過性の行為。ボタンが「立てておけ」の意思表示)。

**番人**: `supplyNotice.test.ts`(レーン単体 8変異赤)/ `server/routes/__tests__/supplySay.routes.test.ts`
(**本番の HTTP 入口から卓のキーストロークまで** 4変異赤)/ `managerSayWake.routes.test.ts`(起こす判断
そのもの 3変異赤 — 「後任が『安全側に締める』として 404 に戻す」のが一番あり得る退行なので、200 ではなく
**spawn が試みられたこと**を見ている)。⚠ 実装中に実測した本物の欠陥: `flushSupplyNotices` の
...
| **S2** all-workers-down | `engine.notified.has('all-workers-down')`(= running・live worker 0・誰も乗っていない doing の swarm branch カードが `UNATTENDED_DOING_ALARM_MS`=36秒以上残る(名簿外の担当が worktree で動いていれば数えない)、`swarmOrchestrator.ts` の `fireFatalNotifications` 2c)。毎 tick | T3 | inbox へ「どう復旧しますか?」。receiptKey=`S2:<path>:all-workers-down` | `swarmOverseer.ts:829-849` |
...
| **S4'** worker 自由文質問(engine 直検知 — overseer OFF でも動く) | monitor の `classifyOutput(screen, runtimeKind)` が `'question'`。**2026-08-03 から両ランタイム**: PTY は TUI 画面(`detectFreeTextQuestion` — idle footer+空入力箱+末尾`?`)、**SDK は pool status head `[sdk session waiting]` + 蒸留 tail の末尾発話が `?`/`？`で終わる**(`detectSdkFreeTextQuestion`。'working'=turn 中・'quota-parked'=rate-limit の腕・`[tool]`/`API Error`/`[compacted]` のマーカー行は発話でない — マーカー行自身が `?` で終わる形(tool error の "Did you mean git?" 等)も除外)。seam は `detectWorkerFreeTextQuestion(kind, out)` 1 本 — それ以前は SDK worker の散文質問は**永遠に 'normal'** で、通常 stall として nudge→回収されていた。overseer ON かつ心拍 blocked のときだけこの腕は譲る(S4 が起票 — MF1) | T3(LLM なし・bare raise) | inbox へ質問文そのまま + worker の**完全な住所**(runtime+handle — 回答は `deliverAnswerToWorker` で両ランタイムに配達)。QUESTION_GRACE 超過で blocked に park(枠は解放・質問は inbox に残る) | `swarmOrchestrator.ts`(monitor 'question' 腕)+`swarmQuestions.ts` |
...
- ⚠️ **規約が閉じた弁の代わりに指す逃げ道は、実在するものだけ**(2026-07-19 差し戻しで是正)。
  ...
  **「専門レビュアー(別カード)」= Board の未実装 todo カード**を指しており、テストがその文言を
  ...
  - ⚠️ **必ず質問の形**(`?`/`？` を入れるのが確実。「〜ですか」等の疑問形も通る)。
    ...
    大脳の日次上限を1回無駄に消費する)。
  - ⚠️ **監督は既定 OFF**(再起動で必ず OFF — ~~in-memory~~ 2026-07-22 card 2 以降は正確には
    「値は `engine.json` に永続されるが boot では読み戻さない」。挙動は変わらず必ず OFF に
    戻る点は今も真。詳細は 01 章 §7.4・ENGINE_PERSISTENCE_PLAN.md §2)。その場合 S4 レーンは死んでいて、
...
- **`fireFatalNotifications` の 2 系統**(`swarmOrchestrator.ts:5887-5954`):
  ...
  - **`all-workers-down` が数えるのは「誰も乗っていない」doing カードだけ**(2026-09-24・2回目の直し): エンジンの名簿(`engine.workers`)の生存数0だけでは「誰も作業していない」とは言えない — 司令官が手で立てた担当(`POST /api/swarm/worker`)や Board 実行の担当は名簿に入らない。実測: 07:38:45 自動更新の再起動 → 担当は再起動で終了 → 07:41 司令官が代わりの担当を手で立てて直させていた → 3分の猶予が切れた時点で名簿は0人なので「全員止まった」が鳴った(上の猶予は時間だけで、名簿外の担当を見ていなかったのが取りこぼしの理由)。いまは `detectAnomalies` が毎 pass(稼働中かつ生存 worker 0 のときだけ)doing の swarm カードごとに作業場所(worktree)を `deskAttended` で調べ、**まだカードを進めている** desk も `unownedOrphanHold`(生き残り claude プロセス。ただし保留開始から `STALE_HEARTBEAT_MS`=30分まで)も無いカードを `engine.unattendedDoingSince`(カード→無人になった時刻)に記録する。`fireFatalNotifications` はそこに `UNATTENDED_DOING_ALARM_MS`(= 無人 doing 回収の猶予30秒 + 2 tick = 36秒)以上続けて載っているカードだけを数え、anomaly 行も同じ map・同じ窓で数える。**「進めている」の定義**(`deskRecentlyActiveIn`、`liveDesks.ts`): SDK セッションが `working`/`starting`(ターン中。ただし最後の出来事が `MAX_EXEC_MS`=90分より古ければ数えない — git の U 状態や最初のメッセージを出さない CLI でターンが永久に固まると、mid-task sweep もその席を生きているとみなすので、上限が無いと知らせが永久に鳴らない)、または `waiting`/`quota-parked` でも最後の出来事が `STALE_HEARTBEAT_MS`(30分)以内。PTY はアプリが立てた claude の画面(tag `claude`)で最後の出力が30分以内のときだけ — dev server / `tail -f` / `watch` / 時計入りプロンプトは出力し続けても数えない(それらこそ「実行中のコマンド」なので、「何か実行中なら数える」は使えない)。`exited`/`failed`/reaped は数えない。生きているだけの desk(成果を出さずにターンを終えて `waiting` のまま放置された手動の担当、worktree に開きっぱなしのシェル、止め途中のセッション)は在席に数えない — 数えると知らせが永久に止まる(司令官レビュー 2026-09-24 の MUST-FIX)。無人 doing 回収(`collectUnownedDoing` の `defaultUnownedDeskState`)にも同じ「放置 desk」の形はあるが、あちらは desk を止めて回収する破壊的な操作で、`waiting` の担当は司令官が `POST /api/sdk-session/:id/input` で差し戻す相手でもあるため、判定は変えていない — 放置は30分+36秒後にこの知らせで人に届く。窓は手動ルートがカードを doing にしてから session が登録されるまでの隙間を覆い(desk が一瞬でも現れたら時計はやり直し)、回収 sweep の猶予より必ず長い — 回収が先に todo へ戻す(エンジンが自分で直せる状態は「全員停止」ではない)。**減らしすぎない側**: 知らせ用の desk 検査は `deskAttended`(`defaultDeskAttended`)で、anomaly の unowned-doing 行が使う `deskOccupies`(失敗=在席)とは逆に**失敗=誰もいない**(=従来どおり鳴る)。map が未導出なら従来どおり全 doing カードを数える。停止で時計は捨てる。番人: `swarmOrchestrator.test.ts`「who is still on the card」16本(うち「an idle desk does not attend the card forever」9本 — 固まったターン・`starting`/`quota-parked` の扱い・出力し続けるだけの画面を含む) + `resumeEngines.test.ts`「a replacement worker the commander started …」。**既知の限界(記録のみ・未対処)**: 30分を超えるバックグラウンド処理を起動してターンを終えた worker は `waiting` のまま最後の出来事が止まるため、30分36秒で「全員止まった」の誤報になりうる。同様に、普通のシェルに手で打った `claude` も在席に数えない — 前面プロセス名は native インストーラ版だと版番号(実測 `2.1.281`)、npm 版だと `node`(= vite / dev server と区別できない)で、`claude` にはならないため。誤るのは「鳴る」側(30分36秒後)だけで、知らせを黙らせる側には誤らない。
