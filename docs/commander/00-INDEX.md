...
| worker が **SDK** で上がる/上がらない・タイルに端末が出ない・`terminalId` が空 | 02 章 §2.4-0(**ランタイム2種**・レコードの `runtime` 不在⇒pty／**ダイヤル不在⇒sdk が 0802 に dispatch まで届いた**=未設定なら SDK worker。⚠0.11.47 までは盤面 ON/実効 PTY だった・降格は正常・生死は `isSdkSessionLive`)/ 設計は `docs/SDK_WORKER_MIGRATION_PLAN.md` |
| **ダイヤルは `sdk` なのに worker が全部 PTY で上がる**(とくに配布した `.app` で) | まず **`fellBackBecause` の文面**(spawn レスポンス → Swarm タブが表示)。**サーバログは見えない**(配布版の Hono は fork された子プロセス)。既知の一撃は**2つとも「配布ビルドでだけ死ぬ」形**: ① **CJS バンドルに `import.meta` が無い**ため guard hook のロードが TypeError → fail-closed で preflight 全落ち(0801 `dd311acc`)。② **CJS バンドルから ESM 専用の SDK を `require()`** していたため spawn が毎回 `ERR_REQUIRE_ESM` で即死 → 全数 PTY へ降格(0802 `e26d5efb`。**0.11.47/0.11.48 はこの状態で出荷**され、SDK ランタイムは製品として一度も起動していない)。**どちらも dev と vitest では原理的に再現しない** → 02 章 §2.4-0 |
