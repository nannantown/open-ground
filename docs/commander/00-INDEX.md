...
| worker が **SDK** で上がる/上がらない・タイルに端末が出ない・`terminalId` が空 | 02 章 §2.4-0(**ランタイム2種**・不在⇒pty・降格は正常・生死は `isSdkSessionLive`)/ 設計は `docs/SDK_WORKER_MIGRATION_PLAN.md` |
| **ダイヤルは `sdk` なのに worker が全部 PTY で上がる**(とくに配布した `.app` で) | まず **`fellBackBecause` の文面**(spawn レスポンス → Swarm タブが表示)。**サーバログは見えない**(配布版の Hono は fork された子プロセス)。既知の一撃: **CJS バンドルに `import.meta` が無い**ため guard hook のロードが TypeError → fail-closed で preflight 全落ち(0801 `dd311acc`。dev と vitest では原理的に再現しない) → 02 章 §2.4-0 |
