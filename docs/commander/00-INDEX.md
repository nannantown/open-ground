...
Persona/proxy answers and self-supply are removed. Engine integration remains commander-owned; its dormant panel and mutating implementation are now deleted.
...
**SDK-only manager (2026-09-21):** all new managers use SDK for owners and public
users. The runtime setting/switch and PTY launcher are removed. Legacy in-flight
PTY desks remain addressable until stopped; conversations and task data remain.
Earlier dial/fallback notes below are historical. See SIMPLIFICATION.md and 03 §2.3.
...
**追記(2026-09-18・常駐卓の文脈上限)**: 司令官・補給官の卓は**無条件に resume されなくなった**。文脈量(`sessionContextTokens`)が `Settings.deskContextCapTokens`(既定 300,000・0=無効)以上なら、司令官は spawn 時に**新しい会話で作り直し**(復帰指示はそのまま=「状況」から読み直す)、補給官は生きた卓に空き時だけ `/compact` を1回送る。根拠=5時間枠の消費の大半がキャッシュ読み込み(常駐卓が巨大な文脈を毎ターン読み直していた)。「司令官の会話が短くなった/前回の話を覚えていない」は**正常**。正典 05 章 §10.6。

**追記(2026-09-22・型が働き方も決める / 卓の消費が内訳に出る)**: ① 難易度の型が worker の**チーム規模とレビュー方針**も決めるようになった(`/order ゴール` 直後の `【難易度: <型>】` 文 — 04 章 §5.9 追記 2026-09-22)。`touch` の worker がサブエージェントや敵対レビューを使わないのは**正常**。② 使用量パネルの 7 日内訳に**「司令官」「補給官」の行**が出る(`~/.openground/swarm-desks.json` の卓台帳 — 卓の起動時に session id を記録。台帳より前の卓は「自分のプロジェクト(自分の作業、または記録前の卓)」に残る = 過去は捏造しない)。
...
| **Ground カードが「Running」のまま**(worker の心拍は何時間も前・実際は何も動いていない) | 02 章 **§3.3 末尾**(2026-09-17) — 仕事を終えて統合待ちの SDK worker はプールに常駐するので「ハンドルがある」は「動いている」ではない。lamp は pane の status `working` で数える。裏取りは `GET /api/terminal/active` でその worktree の status(`waiting` なら止まっている)と `GET /api/swarm/workers` の `phase` / `heartbeatAt` |
| **カードを積んだのに自動で配られない / 未着手のまま動かない** | 01 章 **§4.3 ゲート⑦**(2026-09-11) / **ゲート⑧**(2026-09-26 — Board で薄いカード = 下書き。外すまで自動では配らない) — **内容(完了条件)が空のカードは配らない**。ジャーナルに「内容(完了条件)が空のカード N 件は自動では配りません」が1行出る。内容を書けば次の周回で拾う。急ぐならカード詳細の「実行」(手動はゲート対象外)。0.11.106 より前は逆の事故が起きた(空のカードが8秒で配布され、完了条件なしで着手) |
...
| **上限(週次含む)が明けても worker が復活しない / カードは「実行中」なのに稼働 0 / 実行中カードの詳細に実行ボタンが出る** | 01 章 **§7.4d**(2026-09-01) — unowned な作業途中カードは `collectUnownedDoing` が 30 秒猶予ののち todo へ回収(quota-parked 卓は停止+salvage 込み)、上限明けは冷却テーブルが自動で解く。ドロワーは union リスト(`GET /api/swarm/workers`)で生きた卓の画面 or 中断の説明を出す(0.11.102)。**サーバ強制終了で生き残った claude(孤児)がその worktree の session を握っている間は、回収は意図的に保留される**(2026-09-24 — ジャーナルの「回収を保留します」warn と anomalies の `unowned-doing` が目印。孤児が終われば約30秒で回収、Windows は保留しない)|
| **worker が `ready:true` / phase done を打っているのに review に上がらない・`worker stalled — nudged` が出る**(成果は子リポ / 入れ子 worktree にある) | 02 章 **§5.2 改訂 2026-09-13** — promote の `hasWork` は**親ブランチ + worktree 直下の入れ子リポ**を数える(0.11.110〜)。それ以前は親のみで、sns-hub(親フォルダ型プロジェクト)の完成品が 26 分放置された。promote 行の `成果の所在: 入れ子リポ …` を読む。親にも入れ子にも本当に無いなら **ready-without-work**: 初見で鐘 + journal、3 分 nudge なしで保持 → blocked(01 章 §7.4 行 4c) |
...
| **社長の窓口が空いているのに司令官の返事が届かず、再起動のあとに「(約N分前の返事)」でまとめて届く** | 06 章 **§1.13**(2026-09-26)— 社長の直前の答えが番号付きリスト(1. 2. 3.)で終わると、bypass 表示の「shift+tab」を footer と読んでメニューと誤判定し、配達を保留し続けた。`detectMenu` を ❯ カーソル必須に戻し、カーソル行の下に入力欄(区切り線直下の ❯)があれば会話履歴とみなす。保留が5分を超えるとジャーナルに「社長の窓口への配達をN分保留中: <理由>」が1行出る。再現は `scripts/probe-supply-numbered-list.mts` |
| **社長の入力欄に司令官の返事/エンジンの知らせが入ったまま送信されない / 返事が社長に届かない** | 06 章 **§1.12**(2026-09-26)— 狭い席では claude が長い入力の末尾数行しか描かず、Enter の番人が「全文一致」を待って押さなかった(末尾一致を許す+Ctrl+A で先頭も自分の文か確かめる+全文が見えない時は「自分の貼り付け以降、配達以外の入力が端末に1バイトも無い」ことをサーバの記録で確かめてから押す=`terminalForeignInput`+貼る直前に空欄を読み直す。スマホの Remote Control からの入力は記録に乗らない)。届かない方はパスの大文字小文字違い(`deskKey` を realpath 化)。再現は `scripts/probe-supply-delivery.mts`、実卓の画面は `scripts/peek-desk.mts` |
...
| **生成の途中だった卓・worker**(2026-09-23 追記) | ◐ **その1ターンだけ失われ、続きから再開する**。自動アップデートは 2026-09-23 から **Claude の生成中でも待たずに再起動する**(`electron/autoUpdatePolicy.js` — 待つのはオーナーがウィンドウを操作中(直近3分のキー/マウス入力)と、再開の仕組みが無いもの=オーナー自身のターミナル・裏方の単発 claude が動いている間の最大30分だけ)ので、この行は日常になる。社長・司令官は `supplyDesired`/`managerDesired` から会話ごと resume、作業途中の worker は `adoptResumeCandidates` が**同じ worktree・同じ session** で resume する(コミット前の変更は残る — 上の行の「card 4・未着手」は旧記述で、実装済み)。twin が出ないのは通常の停止の場合で、サーバが SIGKILL された(猶予切れ・OOM・クラッシュ)ときは作業中の claude が孤児として生き残る — resume はそれを `ps` で見つけて断り、unowned-doing 回収も孤児が終わるまで worktree を片付けず再配車しない(2026-09-24 — 01 章 §7.4d。`ps` の無い Windows では確かめられないので従来どおり回収し、twin の余地が残る)。孤児はそのまま自分の作業を続け、終わったあとで通常どおり回収される。更新直後(transcript の最終更新から10秒以内)の起動でも、その session を握る生きた claude が居ず(`ps` で確認)、窓が過ぎるまで待って測り直しても書き込みが進んでいなければ resume する(2026-09-24 以前は時刻の近さだけで断っていた — 01 章 §「engine.workers に居ない worker」・05 章)。transcript が読めない worker は unowned-doing 回収が未コミット分をブランチへコミットしてから todo へ戻す。⚠ 自動運転 OFF のプロジェクトでは worker の adopt も unowned-doing 回収も走らない(`collectUnownedDoing` は `runDispatchPass` の中だけで、`!engine.running` で先に戻る)— worktree と未コミット変更はそのまま残り、司令官の「状況」の死んだ worker 再起動を待つ。社長は返事の途中で切れたら `SUPPLY_RESUME_INJECTION` によりオーナーの未回答の発言に Board を確かめてから答え直す |
...
2. **自己申告 ready を信じず再検証。** エンジンの promote すら「見えるコミットがある」(`probeHasWork` = 親ブランチ `commitsAhead > 0` **または** worktree 直下の入れ子リポの commit — 2026-09-13 改訂)を必須にしている — 「done true」の心拍は宣言であって証明ではない。司令塔も同じ基準で `git rev-list --count origin/main..<branch>` を打つ(02 章 §5.2・§8)。
...
| `overseer` の ON | ⚠️ 現プロセス限り(ただし**記憶はされ、社長に頼めば戻せる**) | 値は `engine.json` に書かれ**記憶される**が、boot はそれを読み戻して arm**しない** — 再起動で必ず OFF(01 章 §7.4)。「前回 ON だったから今も ON」は overseer に限って今も成立しない(理由: 大脳 PTY 起動・worker PTY 注入・janitor の破壊的操作を直接駆動し、再起動が代替の無い kill switch 層 — OVERSEER_DESIGN.md K2/L9-③・ENGINE_PERSISTENCE_PLAN.md §2)。**2026-07-24 card 2b** で GET に `overseerRemembered`(engine.json の生値・表示専用で arm 入力にはならない)が増えた。当初は画面の「前回は監督もオンでした → [戻す] [×]」バナーで見せていたが、**2026-09-24 にバナーは画面から撤去**(オーナー決定 — 状況の監視は画面で操作しない)。今は `overseerRemembered && !overseer` を**社長が状況報告で平易に一言伝え**(自分からは戻さない)、オーナーが頼んだときだけ社長が `POST …/overseer {enabled:true}` で戻して `.overseer` を読み戻す(`skills/supply/SKILL.md`「Owner-requested settings」)。オーナーが「戻さなくていい」と答えたら、社長が専用の `POST …/overseer/dismiss` で記録を消し `.overseerRemembered` が false になったのを読み戻す(`…/overseer {enabled:false}` はこの状態では何も書かない — 01 章 §7.4)。(`autoMerge` フィールドは 2026-07-16 撤去 — 司令官の自動起こしはエンジン ON で常時) |
