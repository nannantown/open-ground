...
> センチネルを刻もうとする → 書き込み保護が reject → **500**。**オーナーが何も保存しなくても、
> アプリを起動しただけでこうなる**(番人 `server/routes/__tests__/groundLoadOnBrokenSettings.test.ts`)。
...
司令官の卓は今どの状態?(managerPresence・now 注入)
  ...
  │                                  ★2026-08-14 ここで**黙らない**: 統合待ちが 40 分以上滞留していれば
  │                                  `manager-unresponsive` の鐘を**エピソードに1回**鳴らし、卓が SDK なら
  │                                  **1度だけ**閉じて absent 経路に立て直させる(PTY 卓は閉じない)→ return
  │                                  (§7-10。滞留 40 分未満・予算が残っている間は従来どおり鳴らさない)
  │     直前 nudge から interval(10分)未満? ─Yes→ return
  │     ─No→ nudgeManager(生きている卓の PTY へ1行+CR)・nudges+1・lastNudgeAt=now・info ログ
  │           ※1バイトも書けなかった場合は nudges を**増やさず** unaddressable+1(2026-08-04)。
  │             3回連続で書けず、かつ滞留 40 分以上なら同じ `manager-unresponsive` の鐘(理由文は「声をかけられません」側)
...
**パネル構成**: 既定は lens パネル 4 体 — correctness / security / perf / regression(`DEFAULT_REVIEW_LENSES`、:3838-3859)。1 lens = 1 レビュアーで、それぞれ焦点だけ違う read-only プロンプト(`buildReviewPrompt` :4020-4040)。モデルは **2026-09-16 から実行モードに問い合わせる**(`desiredModelEffort(mode, 'reviewer')` — 既定 optimize では **opus / high**、`max` なら fable、`economy` なら sonnet)。それ以前は `opts.model ?? SWARM_LAUNCH_MODEL` の **fable 固定でモードを一切見ていなかった**ため、本番配線(モデルを渡さない)では **lens の数だけ = 4体の fable `claude`** が1レビューごとに起動する形になっていた。⚠ ただし上の注記のとおり **`deps.review` は 2026-07-15 以降 呼び出し元がゼロ**なので、この穴が実際の消費を生んでいたわけではない — 再配線した日に再発しないよう先に塞いだ、という位置づけ(04 章 §5.9)。希望 tier を決めた後は従来どおり **cooling と使用可能モデルマスクを通して降格解決**(2026-07-13 からは `resolveAvailableTierProbed` 経由 — 未知 tier には起動前プローブが1発入り、壁なら冷却して1段降格。04 章 §5.8)。
...
### 5.9 オフライン保留 — 圏外で声かけ予算を燃やさない・復帰の瞬間に起こす(2026-09-02)

**実測(カフェ)**: 帰宅→移動→カフェで接続。レビュー待ち1件・司令官の卓は生きているのに
「待機中」のまま。通知・10分×3回の声かけ・1時間後の再武装、**全部が API に届かない卓へ
打たれ、全部「無視された」として課金**されていた。復帰時点で予算は尽き、エンジンは設計
どおり黙っていた(無視する卓は人の仕事)。エンジンは「無視する卓」と「落ちた Wi-Fi」を
区別する材料を持っていなかった。

**追加**(`swarmConnectivity.ts` + `runIntegratePass` の司令官反射、idle/stalled 分岐の
予算読み出しの直前):

| 状態 | 動き |
|---|---|
| オフライン(API ホストへの HEAD が投げる/タイムアウト) | **保留**: 声かけを打たない・予算も throttle も減らさない・journal に一度だけ「オフライン — 司令官への声かけを保留」。Board の司令官帯は **「オフライン待ち」** |
| オフライン→オンラインの立ち上がり | **予算・throttle・一回限りのラッチを全部リセットして、その周回で即座に声かけ**(journal「ネット復帰を検知 — 司令官に声かけし直します」)。使い切った予算も戻る |
| プローブ自体が失敗(例外) | **オンライン扱い**(fail-open) — 保留が健全なマシンを黙らせる道具になってはならない |

プローブは 30 秒キャッシュ・3 秒 deadline・**HTTP 応答なら 4xx でもオンライン**(問いは
「往復できるか」であって認可ではない)。既知の穴: キャプティブポータル(カフェのログイン
ページ)は何にでも HTTP を返すので、ログイン前は「オンライン」に読める — 卓のターンが
失敗し、次のプローブ(TTL 後)で保留に戻る。人ではなく TTL で有界。

**やらないこと**: dispatch(worker の新規起動)は触らない(spawn park は別の仕組み)。復帰時に
卓を作り直さない — 既存の卓を起こすだけ。episode の終わり(review が空・卓が稼働)では
保留フラグも一緒に消える。

歯(mutation で赤を実測済み): `swarmOrchestrator.test.ts` の「OFFLINE HOLD」4本(保留を
消す=圏外でも打つ / 復帰リセットを消す=復帰しても黙る / fail-open を消す=プローブ故障で
声かけ停止)、`BoardModule.worker.test.tsx` の「OFFLINE HOLD」1本(帯の分岐を消す=待機中に
戻る)、`swarmConnectivity.test.ts` 4本(4xx=オンライン / 例外=オフライン / deadline / TTL)。
...
9. ~~**`manager-unrevivable` fatal は Swarm ペインの一覧に出ない**~~ → **2026-08-04 に構造ごと解決済み**(記録として残す)。当時はクライアント側に**手書きの許可リスト** `KNOWN_FATAL_EVENTS` があり、サーバの `SwarmFatalEvent` union に対して短かったため `manager-unrevivable` / `guard-unwired` / `engine-resume-suppressed` / `data-integrity` の4種が sanitize で**黙って捨てられて**いた(ペインは「対応することはありません」を描きながら、worker が1体も起動できない状態を隠していた)。**修正は方向の反転**: 許可リストは廃止され、`isRenderableFatalEvent`(`useSwarmEngine.ts`)は**空でない event 名を全部通す** — ラベルが無い event は生の名前で描かれる(知らない行は聞けばよいが、消えた行は永久に知られない)。翻訳漏れの側は `swarmOverseerFatalLabels.test.ts` が **`src/lib/types.ts` の union を実際にパースして** `FATAL_EVENT_LABEL` と突き合わせるので、新しい event を union に足すとラベル+i18n 2言語が**ビルド時に強制**される(登録漏れ=沈黙 ではなく過大近似=テストが赤、CLAUDE.md 戒4)。**新しい fatal を足すときの手順(2026-09-23 更新)**: `types.ts` の union → `swarmNotifications.ts` の `EVENT_LABEL`(OS トースト表題。`Record<SwarmFatalEvent,string>` なので**足し忘れは tsc が落とす**)。これだけ。Swarm 画面の要注意フィード(`SwarmOverseerPane` の `FATAL_EVENT_LABEL`・`useSwarmEngine` の `SwarmFatalEventKind`・番人 `swarmOverseerFatalLabels.test.ts`)は 監督タブごと撤去された(06 §1.7)。fatal は `detail` の平易文がそのまま社長への知らせ(`supplyNotice.ts`)とベルに載るので、**`detail` をオーナー向けの平易文で書く**ことが表示面の義務になった。
10. ~~**卓が固まったまま nudge に応答しない場合、通知は上がらない**~~ → **2026-08-14 に解決済み(本節が予告していた「次カード」がこれ)**。
    **何が起きていたか**: `idle` の卓へは nudge を 3 回(10 分間隔)まで打ち、使い切っても応答が無ければ engine log に 1 行出して**永久に黙る**設計だった。`manager-unrevivable` は「卓を*起動*できない」意味なので流用できず(§2.3・完了条件3)、専用 fatal を新設するには表示面(union+ラベル+i18n)の工事が要るため見送られ、本節は **「通知の到達性としては旧実装より後退」「正しいラベルで届けるまでは次カード送り」** と記録していた。その状態のまま **2026-08-14 に実際に踏んだ**: エンジン ON・review に 2 枚・SDK 卓は生きていて idle・エンジンは nudge を諦め、**オーナーには何も届かなかった**。もう一方の袋小路(**声をかけられない** = `nudgeManager` が 1 バイトも書けない状態が 3 回連続。2026-08-04 の「打ち止めしない un-sent poke」修正で `error` ログ 1 行だけが残っていた経路)も同じ形で無音だった。
    **今の挙動(3点)**:
    - **鐘**: 専用 fatal **`manager-unresponsive`** を新設。発火条件は3つの AND — ①卓は在るが統合していない(`presence==='idle'`、または `'active'` かつ stall 判定)②nudge 予算を使い切った(**または** 声かけ不能が `MAX_MANAGER_NUDGES` 回連続)③**統合待ちが `MANAGER_INTEGRATION_STALL_MS`(40分)以上滞留**。③は `stalled` 判定ではなく**素の滞留時間**を見る(`stalled` は delivery 証拠が皆無だと fail-open で false になる — 卓に ESC を撃つ判断には正しいが、「オーナーに知らせるか」の判断を沈黙で答えてはいけない)。**エピソード(= 待ち行列のバッチ)につき 1 回だけ**鳴る(`managerResume.unresponsiveFatalFired`。review が捌けるか、卓が working と読まれた時にだけリセット — **再 spawn ではリセットしない**)。detail は平易文で 「司令官に「マージ」と声をかけるか、司令官を再起動してください」 まで書く(`managerUnresponsiveDetail`。「応答なし」と「声をかけられない」で文言が変わる)。
    - **卓の作り直し**: 同条件で、かつ卓が **SDK のときだけ**、卓を1度閉じる(`recycleManagerDesk`。閉じたあとは既存の absent 経路が普通に新しい卓を立てる = 起こし方の道は増えない)。**PTY 卓は決して閉じない** — PTY 司令官はオーナー自身の cwd の端末で、オーナーが手で開いて今読んでいるかもしれない(この除外は呼び出し側と `defaultRecycleManagerDesk` の**両方**で判定する。await を挟んで 2 回読むので、間にプールが入れ替わりうる)。こちらも**エピソードにつき 1 回**(`deskRecycled`)— でないと spawn→wedge→kill の緩いループになる。**作り直しが throw しても鐘は必ず鳴る**(順序は recycle → 鐘、例外は握り潰す)。
    - **滞留時計の永続化**: `engine.reviewSeenAt`(= ③の分母)は in-memory だけだったので、**アプリ再起動のたびに 40 分窓がゼロに巻き戻っていた**(同日 3 リリースで 3 回巻き戻った)。`engine.json` の `reviewWaitingSince`(branch→epoch ms)に**集合が変わった時だけ**書き、`resumeEngines` が読み戻す。**明示的な owner ON(`startOrchestrator`)は従来どおり時計を捨てる** — エンジンが OFF だった時間は「統合待ち」ではない(2026-07-22 の差し戻し)。読みは fail-open(壊れた/無い値 ⇒ 時計なし = 従来挙動)、書きも fail-open(ディスク不良は次回 boot の頭出しを失うだけ)。
    **番人**: `swarmOrchestrator.test.ts` の manager-reflex ブロック(鐘は1回だけ/PTY は作り直さない/SDK は1回だけ/throw しても鐘は鳴る/予算が残っている間・滞留 40 分未満は鳴らない/エピソード終了でリセット)+ `swarmReviewDwellPersistence.test.ts`(ディスク側: 変化時だけ書く・捌けたら消す・resume で読み戻す・壊れた値は無視)。**変異で赤を実測済み(7件・2026-08-14)**: 鐘を消す→13赤 / エピソード latch を消す→5赤 / resume の読み戻しを外す→2赤 / PTY 除外を外す→1赤 / 滞留ゲートを外す→2赤 / ディスク書きを外す→5赤 / `startOrchestrator` の時計クリアを外す→1赤。
    **なお** claude が**プロセスとして死ぬ**形は `; exit` フレーミング(`claudeTerminal.ts`)で shell ごと落ちるため `absent` → 従来どおり `manager-unrevivable` に届く。この節が扱うのは「プロセスは生きているが対話プロンプトで固まる」形だけ。また**この「3 回で打ち止め」が実際に成立するのは、自分の nudge のエコーを presence 判定から割り引いているから**(§2.3 の `echoUntil`)。卓へ1行書くと TUI が描き直して `lastOutputAt` が更新されるため、割引が無いと毎回 `active` に化けて予算が 0 に戻り、この行は永久に出ないまま卓を突き続ける(回帰テストで実測 300 分 30 回 → 修正後 3 回)。
