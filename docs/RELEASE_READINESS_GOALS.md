...
3. **GAP-7**: ✅ **決着(2026-07-27 — 実装せず「受容」で決着・開示まで完了)**: Windows には L3(OS サンドボックス)相当が無く、封じ込めが **L4 単層**になる(macOS 限定の `sandbox-exec`)。**実装しない**と決定 — 受益は owner の 1 台のみ(swarm は owner 限定の隠し機能で一般ユーザーに到達経路が無い)に対し、Windows 側は AppContainer が API のみ(前置コマンドが無い)で新規機構の設計から必要、Job Object は資源/UI 制限止まりで FS・資格情報の封じ込めを持たず、Windows Sandbox は Home 非対応・単一インスタンス・ホストのソフト不在で代替不能。代わりに **(a)** 単層である事実と「L4 自体が Windows で未実測」(NEW-3)を [SANDBOX_EXPERIMENT.md](SANDBOX_EXPERIMENT.md) と GAP-10 の開示に明記し、**(b)** GAP-1(swarm 一般開放)で再評価する、を条件とする。
...
- **L3 sandbox / overseer brain の OS 封じ込め**(darwin 限定)— **GAP-7 受容済み(2026-07-27)**: Windows は L4 単層のまま(しかもその L4 の Windows 実効性は未実測 — NEW-3)。**開示は書き込み完了** — 正典 [SANDBOX_EXPERIMENT.md「Windows — there is no L3 here」](SANDBOX_EXPERIMENT.md) + GAP-10 開示文(アプリ内マニュアル Swarm 章・バイリンガル)。GAP-1 で再評価。
- **og-manage コマンダースキルの中身が bash/curl 一色**(実体は `skills/og-manage/SKILL.md` 1 ファイルで、その中の手順が bash/curl のワンライナー — `.sh` 群ではない)— Windows 素の PowerShell では bash が無く、`curl` は `Invoke-WebRequest` の alias(PowerShell 5.1)なので**別 cmdlet に束縛され curl のフラグが通らない**(= curl として機能しない)— **GAP-8 受容済み(2026-07-27)**: PowerShell 対応はせず「コマンダー運用は macOS / Git Bash 前提」を明示する方針。**壊れるのは人間コマンダー運用だけ、ではない(2026-07-28 訂正)**: bash 依存が無いのは**エンジンのサーバ側コード**(worker spawn/統合/janitor/overseer)までで、worker の完了申告に使う心拍は bash スクリプト(`scripts/swarm-beat.sh` が `readyToMerge` の唯一の書き手・worker 心拍の HTTP 経路は無い)。`classifyWorker` の doing→review 昇格の根拠は `ready` **または PTY 死亡**の 2 つ(`swarmOrchestrator.ts:1217`)だが、claude TUI は /order 完了時に通常 exit しない(同 :1196-1200)ので実際には stall 経路が最頻 — bash の無い Windows では worker が自己申告できず stall → 回収 → `commitsAhead>0` で `blocked` へ退避しやすい。よって**無人運転を Windows で回す場合も Git Bash が要る**(かつこの経路自体が Windows 実機で未検証 — 本節冒頭 ⚠️ の 4 点と同じ扱い)。**その Git Bash で足りるかも未検証**(心拍ディレクトリの鍵導出が bash 側の `pwd -P` と Node 側の `resolve` で食い違う懸念 — 詳細は §5 GAP-8 行)。[SWARM_GA_AUDIT.md §1.4 A-1](SWARM_GA_AUDIT.md) も同じ bash 依存を critical と判定(ただし A-1 の前提①「私物・非配布」は陳腐化 — §5 GAP-8 行の注記を参照)。
- `lsof` ポート競合診断(win はスキップ・汎用エラーのみ)— 軽微、非ブロッカー。

### 3.3 swarm 機能そのもののクロスプラットフォーム判定

swarm エンジンの**サーバ側コード**(orchestrator/worker/integrate/janitor/overseer 脳幹)はプラットフォーム中立に書かれている — **「エンジン全体が中立」ではない(2026-07-28 訂正)**: worker が完了を申告する心拍だけは bash スクリプト(`scripts/swarm-beat.sh`)で、TS 側に writer が無い(§3.2 の GAP-8 行 / §5 GAP-8)。公開判定で問うべきは次の 3 点:

- [ ] Windows 実機で worker(= PTY 上の claude)が spawn し心拍を打てるか(GAP-3 の一部 — `~/.claude/swarm-beat.sh` は bash スクリプト。**2026-07-27: Windows 代替は実装しないと決定**(GAP-8 と同判断・コマンダー運用は macOS/Git Bash 前提) — この項目は「無人エンジンが Windows worker で動くか」の実測用に残すが、心拍手段自体の Windows 移植は対象外)。**核心(2026-07-28 訂正)**: 心拍が打てなければ worker は完了を申告できず無人ループが人間待ちになる = **無人運転を Windows で回す場合も Git Bash が要る**
...
> ※ [SWARM_GA_AUDIT.md §6-1](SWARM_GA_AUDIT.md) はこの 3 点に「L4 guard の PowerShell 実効性(NEW-3 — PS 固有構文での evasion 試行)」を項目として追加すべきと提案している。**本カード(GAP-8)の範囲外**なので追加は NEW-3 / GAP-3 側に委ね、ここでは提案の存在だけ記録する(2026-07-28)。
...
|---|---|---|
| **GAP-1** | ✅ **決着(2026-07-27 — オーナー決定「まだ swarm 安定してないのでオーナーだけ」)**: ~~一般開放のゲートモデル未設計~~ → **決定済み: 一般開放しない(オーナー限定継続)**。owner gate で閉じたまま運用し、一般ユーザーが自分のマシンで有効化できる形(認証要件・opt-in UI・同意フロー)は**作らない**。根拠は安定性 — 2026-07-23〜27 だけでも worker が ~1 時間で死ぬ / twin ブランチが 1 カードに 7 本生える / 完了通知が最大 40 分遅れる、が実観測されている | **再評価条件(この 3 点が揃うまで再検討しない)**: ① **worker 死亡の原因解明**(理由不明に死ぬ問題の解消。死因記録は 0.11.37 で実装済み — 次の死亡で原因が判明してから) ② **twin 増殖の停止確認**(0.11.36 の根治後、実運用で再発しないこと) ③ **完了通知遅延の解消**。揃った時点で**新しいカードとして起票し直す**。それまで §1 の「swarm 込み一般公開」宣言と GAP-10 の opt-in 組込みは**保留**。⚠ **再起票する際は GAP-7 の受容(Windows は L4 単層・その L4 も Windows 未実測)を必ず再評価する** — 一般 Windows ユーザーは macOS の 2 層に対し 1 層(しかも未検証)で無人 worker を回すことになるため、Windows で開くなら「L3 実装」か「L4 の Windows 実測(§4.3)」のどちらかを前提条件に含める |
...
| **GAP-10** | **同意の開示の組込みが未完**。開示文書そのものは存在するようになった(アプリ内マニュアル Swarm 章(バイリンガル・§2.1 C-4 骨子と同期)+ Settings の Swarm 実験 hint — 2026-07-17 に autoMerge トグル廃止へ追随済み)が、GAP-1 の opt-in フローに「有効化の場での同意取得」としてまだ組み込まれていない | swarm opt-in(GAP-1 で設計する UI)が §2.1 C-4 開示文を有効化の場で表示して同意を取る。開示内容が SWARM_SAFETY_INVARIANTS.md と矛盾しないことをレビューで確認。**開示に含めるべき内容として GAP-7 の受容(2026-07-27)を収録済み** — 「Windows には OS 側の封じ込め層が無く、暴走時に止める層が 1 つだけ(しかもその 1 つは Windows 実機で未検証)」をマニュアル Swarm 章に明記(バイリンガル)。同じ節で macOS 側の層の実際の範囲も較正済み(書き込み封じ込め + 特定の秘密ファイルの読取り拒否まで。ふつうのファイルは読める・キーチェーン項目は到達可能 — `sandbox.ts` の honest limits) |
...
| **GAP-7** | ✅ **決着(2026-07-27 — 受容 + 開示まで完了)**: ~~L3 を実装するかの判断~~ → **Windows は L3(OS サンドボックス)不在のまま受容** = worker/overseer brain の封じ込めが **L4 単層**。guard header 明記の残存穴(既存 gitconfig alias 等)が Windows では OS 層で拾えず、**さらにその L4 自体が Windows で未実測**(NEW-3 — POSIX lexer で win32/PowerShell 分岐ゼロ)。**GAP-1 の「一般開放しない」決定がこの受容を補強する**(worker/overseer brain は owner 限定の隠し機能で一般ユーザーに到達経路が無い) | 達成: L3 は**実装しない** — AppContainer は API のみ(`CreateAppContainerProfile`・前置コマンド不在)で新規機構の設計から必要、Job Object は資源/UI 制限止まりで FS・資格情報の封じ込め無し(暴走の**停止**には使えても**読ませない**ことはできない)、Windows Sandbox は Home 非対応・単一インスタンス・ホストのソフト不在で代替不能(いずれも一次資料つき)。**開示は書き込み完了**: **(a)** [SANDBOX_EXPERIMENT.md §Windows](SANDBOX_EXPERIMENT.md) に決定・意味・却下理由・再評価条件、**(b)** GAP-10 開示文(アプリ内マニュアル Swarm 章・バイリンガル)に平易文で収録。**(c)** **swarm 一般開放(GAP-1)の入力として再評価**が受容の条件 |
| **GAP-8** | ✅ **受容(方針決定済み・2026-07-27 司令官決定)**: ~~PowerShell 対応するかの判断~~ → **PowerShell 対応はしない。推奨環境を明示する**。og-manage は swarm の一部 = owner 限定なので Windows 対応の受益者が現状ゼロ。心拍スクリプト(`swarm-beat.sh`)の Windows 代替も**同じ判断で実装しない**。⚠ **2026-07-28 訂正 — 「エンジンの無人運転は Windows でも無傷」は過大主張だった**: bash 依存が無いのは**サーバ側コード**(worker spawn/統合/janitor/overseer)まで。worker の完了申告に使う心拍は bash スクリプトで(`scripts/swarm-beat.sh` が `readyToMerge` の唯一の書き手・worker 心拍の HTTP 経路は無く `POST /api/swarm/manager/beat` は司令官卓用)、`classifyWorker` の doing→review 昇格の根拠は `ready` **または PTY 死亡**の 2 つ(`swarmOrchestrator.ts:1217`)— ただし claude TUI は /order 完了時に通常 exit しない(同 `:1196-1200`)ので実際には stall 経路が最頻。bash の無い Windows では worker が commit できても自己申告できず stall→回収→`commitsAhead>0` で `blocked` へ退避しやすい = **無人運転を Windows で回す場合も Git Bash が要る**(かつこの経路自体が Windows 実機で未検証 — §3.2 冒頭 ⚠️ と [DISTRIBUTION.md §6](DISTRIBUTION.md) の caveats)。**しかも Git Bash で足りるかは未検証**: 心拍ディレクトリの鍵を bash 側(`scripts/openground-swarm-lib.sh:23-29` の `sw_repokey` — `cd "$cdir" && pwd -P` を sha1)と Node 側(`swarmOrchestrator.ts:3947-3973` の `swarmRepoKey` — `canonicalize(resolve(...))` を sha1)が**独立に導出して文字列一致を前提**にしている(Node 側のコメントが「exactly like swarm-beat's」と依存を自認)。Git Bash(MSYS)の `pwd -P` は `/c/Users/…`、Node の `resolve` は `C:\Users\…` なので sha1 の入力が変わり鍵が割れうる = worker が書く `<keyA>/` と engine が読む `<keyB>/` が別ディレクトリになる。加えて `sw_repokey` は `shasum` に依存し、無ければ `scripts/swarm-beat.sh:27` が exit 1 で**心拍そのものがゼロ**(Git for Windows の `shasum` 同梱可否は未確認)。**Windows 実測時はここを最初に見る。**[SWARM_GA_AUDIT.md §1.4 A-1](SWARM_GA_AUDIT.md) も同じ bash 依存を verdict: breaks / critical と判定(A-1 の結論はさらに広く「macOS の非開発者環境にも同じ穴が開く → GAP-8 を全 OS の非開発者環境へスコープ拡大すべき」まで含む)。なお同 A-1 の前提①「`swarm-beat.sh` は開発者の私物で配布しない」は**陳腐化**(`swarmToolingInstall.ts:73` + electron-builder の `build.files`(`package.json:104`)で boot 時に自己配備済み) — **配備問題は解決済み・bash 依存は別件で未解決**(2026-07-28 に [SWARM_GA_AUDIT.md](SWARM_GA_AUDIT.md) 側の §1.4 A-1 と §4 NEW-4 行へ追記済み。同書 §0 の 🔴 件数と §5 可用性行は旧前提のままで、再評価は別カード) | 達成: 「コマンダー運用(og-manage)は macOS / Git Bash 環境が前提。素の PowerShell では動かない。無人運転もエンジンのサーバ側コードに bash 依存は無いが心拍が bash なので Git Bash が要る(かつ Windows 実機では未検証)」を [DISTRIBUTION.md §6](DISTRIBUTION.md) と [commander/00-INDEX.md](commander/00-INDEX.md) に明記済み(心拍スクリプトの但し書き込み) |
...
- [x] GAP-4/7/8 の判断カードがすべて「決着」— **2026-07-27 に 3 件とも受容で決着**(GAP-4 = 証明書買わない・初回警告を明記 / GAP-7 = Windows L4 単層を受容 / GAP-8 = og-manage は macOS・Git Bash 前提を明示)。GAP-5 は 2026-07-16 決着済み。※ **GAP-4/7/8 の開示文はいずれも書き込み済み** — GAP-4 = README + `landing/index.html` + [DISTRIBUTION.md §6](DISTRIBUTION.md)、GAP-7 = 2026-07-28 完了([SANDBOX_EXPERIMENT.md §Windows](SANDBOX_EXPERIMENT.md) + アプリ内マニュアル Swarm 章)、GAP-8 = 2026-07-27 書き込み・**2026-07-28 に訂正**(「無人運転は Windows でも無傷」の過大主張を除去)して現行版。
