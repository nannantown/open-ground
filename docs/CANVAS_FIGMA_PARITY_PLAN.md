...
2026-06-12 開始。ゴール: **per-project Canvas タブを Figma の design editor と本質的に同等にする**。
...
- [ ] `npm test`（vitest 全件）緑 / `npx tsc --noEmit` 0 error / `npm run lint` 0 error / `npm run test:e2e` 緑
- [ ] 下のチェックリスト全項目が、実アプリ（dev:alt + Playwright スクショ）で確認済み

## Expected Behavior チェックリスト

### A. シェル（Figma 丸パクリのドック型 3 ペイン）
- [ ] Canvas タブを開くと **左に常設サイドバー**（240px・全高）: 上に **Pages セクション**（= 旧 CanvasTabBar のキャンバス一覧。追加 + / リネーム / 切替 / 削除）、下に **Layers ツリー**（残り高さ全部・スクロール）
- [ ] **右に常設サイドバー**（240px・全高）= Design パネル。**未選択時も消えない**（canvas 情報を表示）
- [ ] 中央キャンバスは flex-1。ToolPalette は下中央の水平ピル（Figma UI3 風）
- [ ] 上部バーに **ズーム % 表示+コントロール**（クリックでメニュー: zoom to fit / 100% 等）
- [ ] `⌘\` で両サイドバーをトグル（focus mode）
- [ ] Pages のリネーム入力に IME ガード（isComposing）— 既存バグ修正

### B. Layers パネル（使いやすさ = Figma 同等）
- [ ] 行ホバー → キャンバス上の該当要素に青アウトライン（hover sync）。逆方向（canvas hover → 行ハイライト）も
- [ ] キャンバスで選択 → パネルが**祖先を自動展開してスクロール表示**（reveal）
- [ ] ↑/↓ で行移動、⇧+クリック/⇧↑↓ で**範囲選択**、←/→ で折りたたみ/展開
- [ ] ドラッグで **コンテナ行の上にドロップ → 中に入る**（行中央バンド + コンテナハイライト。空/折りたたみフレームにも入れられる）
- [ ] 検索フィールドで名前フィルタ
- [ ] オートレイアウトフレームに方向アイコン（→/↓）表示
- [ ] z-nudge（Alt+矢印/ボタン）がネスト時も正しく効く — 既存バグ（行 index vs 配列 index）修正

### C. オートレイアウト エンジン v2
- [ ] **主軸 justify**: start / center / end / **space-between** — 「中央にちゃんと均等配置」ができる
- [ ] **Hug contents**（フレームが子に合わせて縮む/伸びる）を軸ごとに設定可
- [ ] **Fill container**（子がフレームいっぱいに伸びる）を子ごと・軸ごとに設定可
- [ ] **per-side padding**（上下左右独立。リンクトグルで一括も）
- [ ] hidden の子はスロットを占有しない（Figma 同様詰まる）
- [ ] コメントピンはレイアウト対象外
- [ ] エンジンは引き続き純粋+冪等（canvasAutoLayout.test.ts の不変条件を維持・拡張）
- [ ] **子の順序 = 配列順（z順）に統一**（Figma と同じ「フロー順 = レイヤー順」）。旧ファイルは読み込み時に位置順へ正規化して見た目を変えない

### D. オートレイアウト インタラクション
- [ ] レイアウトフレーム内の子をドラッグ → **挿入インジケータ（青バー）+ 兄弟がよける**、ドロップで配列順が変わる
- [ ] 外の要素をレイアウトフレームにドラッグイン → 挿入位置に入る / 子をドラッグアウト → フローから抜ける
- [ ] レイアウトフレーム上で新規作成/ペースト → フローに挿入される
- [ ] フレームをリサイズ中も子が**ライブで再配置**される（pointer-up 待ちしない）
- [ ] テキストが伸びたら再フロー（テキスト実測サイズを width/height に保存して footprint を正す）
- [ ] ⇧A: 既存挙動維持（フレーム→その場で有効化 / その他→ラップ）+ ⌥⇧A 解除

### E. インスペクタ（Figma Design パネル同等）
- [ ] **X / Y 入力欄**（W/H/rotation と並ぶ Figma 配置）
- [ ] パネル最上部に**整列ボタン行**（旧 AlignBar を吸収。レイアウト子では非表示=Figma 同様）
- [ ] オートレイアウトセクション: 方向トグル + gap + per-side padding 展開 + **3×3 アライングリッド**（justify×align を書く）+ 軸ごと Hug/Fixed ドロップダウン + プレーンフレームに「+ オートレイアウト追加」
- [ ] **複数選択でも表示**: 共通フィールドは一括編集、値が違えば「Mixed」プレースホルダ
- [ ] NumberInput はキーストロークでなく blur/Enter でクランプ（入力中に値が戦わない）

### F. 回転・リサイズ（Figma 式セレクション）
- [ ] **回転ドット廃止**。選択要素の**角の少し外側をホバー → 回転カーソル**（角ごとに向きが変わる曲がり矢印）、ドラッグで中心回転、⇧で15°スナップ、**回転中は角度バッジ**表示
- [ ] **リサイズハンドル 8 個**（4隅の□ + 4辺）。回転に追従したカーソル向き。⇧=比率維持 / ⌥=中心から（既存数式 resizeRotatedBR を全ハンドルに一般化）
- [ ] リサイズ/選択中に **W×H サイズバッジ**を選択枠の下に表示
- [ ] Esc でドラッグ中の回転/リサイズをキャンセル

### G. 検証インフラ
- [ ] InfiniteCanvas の要素ラッパに `data-element-id`（e2e セレクタ）
- [ ] `e2e/canvas.spec.ts` 新設: ⇧A フロー / 回転カーソル / 8 ハンドルリサイズ / Layers ドラッグ / justify 反映 を実マウスで検証
- [ ] dev:alt（隔離 OPENGROUND_HOME）+ Playwright スクショで全チェック項目を目視確認し、このファイルの ✓ を埋める
